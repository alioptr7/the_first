...
from ..models.model_schemas import LogEntry, SystemHealth, SystemStats
...
@router.get(
    "/health", response_model=SystemHealth, dependencies=[]
)  # Empty dependencies to override the global security
async def get_system_health(db: AsyncSession = Depends(get_db)):
    ...
    return SystemHealth(
        ...
        components={"database": db_status, "elasticsearch": es_status},
        components_stats={
...
async def get_system_logs(
    ...
    """
    Get system logs from the database or a logging service.
    Requires admin privileges.
    """
    return await system_service.get_logs(
        db, start_date=start_date, end_date=end_date, level=level, limit=limit
    )