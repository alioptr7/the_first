...
from response_network.api.schemas.request import (
...
from response_network.api.crud import requests as request_service
...
async def get_request(
    request_id: int,
    request: Request,  # FastAPI injection
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    """
    db_request = await request_service.get_request(db, request_id)
    if not db_request:
        raise HTTPException(status_code=404, detail="Request not found")
    
    # Check if Claude Sonnet 3.5 is enabled
    if request.state.enable_claude_sonnet_3_5:
        db_request.model = "sonnet-3.5"  # Update model to Sonnet 3.5
